  ...
  return (
    ...
    max-w-screen-sm bg-white/95 border rounded-xl shadow-lg z-50">
      <div className="flex justify-around py-2 cursor-pointer">
        <Link href="/auth/test" className="flex flex-col items-center text-xs cursor-pointer">
          <House className="" />
          <span>ホーム</span>
        </Link>
        ...
        <Link href="/auth/forgot-password" className="flex flex-col items-center text-xs">
          <Table />
          <span>マイページ</span>
        </Link>
        <Link href="/auth/register" className="flex flex-col items-center text-xs">
          <Table2 />
          <span>残高</span>
        </Link>
      </div>
